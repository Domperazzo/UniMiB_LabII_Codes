...
int main(int argc, char **argv) {
  ...
  std::ifstream errori;
  errori.open("scarti.txt", std::ios::in);
  std::vector<double> v_err;
  while (true) {
      double err;
      errori >> err;
      if(errori.eof()==true) break;
      v_err.push_back(err);
  }
  errori.close();
...
for (int i = 0; i < v_x.size(); i++) {
  ...
  grafico.SetPointError(i, 0., 0.03064);
}

TF1 modello("modello", "(10.1*x*[0])/sqrt(1+x^2*[0]^2*102.01)");
...
grafico.SetTitle("Funzione di trasferimento circuito RC");
