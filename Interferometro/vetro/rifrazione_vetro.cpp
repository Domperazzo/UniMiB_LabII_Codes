...
int main (int argc, char ** argv){
    ...
    ifstream f;
    f.open ("vetro_indice.txt", std::ios::in);
    ...
    c1->cd();
    h_eventi->GetXaxis ()->SetTitle ("Indice rifrazione");
    h_eventi->GetYaxis ()->SetTitle ("Frequenza eventi");
    h_eventi->SetFillColor (kTeal +2);
    h_eventi->SetLineColor (kGray);
    h_eventi->SetTitle("Indice rifrazione vetro");
    h_eventi->GetFunction ("model")->SetLineColor (kRed);
    h_eventi->Draw ();
    c1->Modified();
    c1->Update();
    c1->SaveAs("vetro_indice.pdf") ;
